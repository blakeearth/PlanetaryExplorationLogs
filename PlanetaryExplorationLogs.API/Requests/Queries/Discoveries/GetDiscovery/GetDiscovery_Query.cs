...
public class GetDiscovery_Query : RequestBase<string>
{
	private readonly string _someFilter;

	public GetDiscovery_Query(PlanetExplorationDbContext context, string someFilter)
		: base(context)
	{
	...
	public override IValidator Validator => new GetDiscovery_Validator(DbContext, _someFilter);

	// The handler is mandatory to have for every query
	public override IHandler<string> Handler => new GetDiscovery_Handler(DbContext, _someFilter);
}

// The handler class is responsible for executing the query
public class GetDiscovery_Handler : HandlerBase<string>
{
	private readonly string _someFilter;

	public GetDiscovery_Handler(PlanetExplorationDbContext context, string someFilter)
		: base(context)
	{
		_someFilter = someFilter;
	}
...
public class GetDiscovery_Validator : ValidatorBase
{
	...
	public GetDiscovery_Validator(PlanetExplorationDbContext context, string someFilter)
		: base(context)
	{
		_someFilter = someFilter;
	}
